...
public class EraAuthorSpider {
...

    {
        try {
            writeToFile = new WriteToFile(
                        "D:\\project\\idea_project\\ancient-poetry\\ancient-poetry-common\\src\\main\\resources\\author_era_urls.txt");
    ...
    public int spiderUrl(String url){
        ...
        try {
            ...
            for (int j = 0; j < authorNames.size(); j++) {
                temp = authorNames.get(j).text() + " #" + authorIntroduce.get(j * 2 + 1).text() + " #https://so.gushiwen.org" +
                        authorArticleUrl.get(j * 3 + 2).attr("href") + " #"+ authorArticleUrl.get(j * 3 + 2).text().substring(1);
                System.out.println(temp);
                writeToFile.writeDataToFile("五代 #" + temp + "\r\n");
    ...
    public static void main(String[] args) {
        ...
        EraAuthorSpider eraAuthorSpider = new EraAuthorSpider();
        for (int i = 1; i <= 15; i++) {
            eraAuthorSpider.spiderUrl("https://so.gushiwen.org/authors/Default.aspx?p=" + i + "&c=%E4%BA%94%E4%BB%A3");
        }
